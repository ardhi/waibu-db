...
async function getSchemaExt (modelName, view, options = {}) {
  ...
  const schema = pick(model, ['name', 'properties', 'indexes', 'disabled', 'attachment', 'sortables', 'view'])
  const base = path.basename(model.file, path.extname(model.file))
  let ext = await readConfig(`${model.plugin.ns}:/extend/waibuDb/modelName/${base}.*`, { ignoreError: true, options })
  const over = await readConfig(`main:/extend/waibuDb/extend/${model.plugin.ns}/modelName/${base}.*`, { ignoreError: true, options })
