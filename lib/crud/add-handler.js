 ...
 * @returns {Promise<string>} Renders the add view
 */
async function addHandler (opts = {}) {
  let {
    req, reply, model, params = {}, template, addOnsHandler,
  ...
  if (req.method === 'GET') {
    ...
    if (req.query.mode === 'clone' && req.query.id) {
      const resp = await getRecord({ model, req, id: req.query.id, options: { fields: map(schema.properties, 'name'), ..._opts } })
